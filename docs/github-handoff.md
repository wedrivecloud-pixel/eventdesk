...
The initial reviewed foundation was promoted to `main` after owner approval at
`7800f86a96f5c4037c65d0284fa80579711ed7e6`. The repository remains private.

## September 12 feature update

The latest beta features have been ported into this PostgreSQL/Better Auth source:
package photos before the first save, new/existing proposal contact selection,
and optional client proposal discount entry. Discount entry is off unless the
owner explicitly enables it; hiding the input preserves agreed discounts.

This update is based on the existing GitHub tree, preserving its history. It also
includes the browser regression tests and feature QA notes in `docs/`. Local
type checking, unit tests, global-search tests, source credential scan and builds
passed. GitHub validation must pass before promotion. This source update does not
complete the outstanding public-launch infrastructure or migrate beta records.
