...
function createAuth() {
    ...
    return betterAuth({
      appName:'EventDesk', baseURL:appOrigin(), secret,
      ...
      emailAndPassword:{
        ...
        sendResetPassword:async({user,url})=>sendAccountEmail(user.email,'Reset your EventDesk password',url),
      },
      emailVerification:{
        sendOnSignUp:true, autoSignInAfterVerification:true,
        sendVerificationEmail:async({user,url})=>sendAccountEmail(user.email,'Verify your EventDesk email',url),
      },
