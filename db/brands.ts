...
import { checkedBrandPresentation } from './brand-presentation';
...
export async function brandAction(bid: unknown, body: Record<string, unknown>) {
  ...
  if (body.action === 'save_primary_brand_details') {
    if (!body.data || typeof body.data !== 'object' || Array.isArray(body.data))
      throw Error('Enter the brand details.');
    const d = body.data as Record<string, unknown>;
    const p = checkedBrandPresentation(
      { ...presentationFromSettings(config.settings), ...d },
      config.resources,
    );
    const name = text(d.name, 'Brand name', 120),
      publicEmail = email(d.email),
      phone = text(d.phone ?? '', 'Phone', 40, false);
    const address = text(d.address ?? '', 'Address', 1000, false),
      website = text(d.website ?? '', 'Website', 500, false);
    if (website) {
      try {
        if (!['https:', 'http:'].includes(new URL(website).protocol))
          throw Error();
      } catch {
        throw Error('Enter a valid https:// or http:// website URL.');
      }
    }
    await db.batch([
      db
        .prepare('UPDATE businesses SET name=?,email=?,phone=? WHERE id=?')
        .bind(name, publicEmail, phone, bid),
      db
        .prepare(
          'INSERT INTO business_settings(business_id,data,updated_at) VALUES(?,?,?) ON CONFLICT(business_id) DO UPDATE SET data=(business_settings.data::jsonb || excluded.data::jsonb)::text,updated_at=excluded.updated_at',
        )
        .bind(
          bid,
          JSON.stringify({
            ...p,
            trustIndicators: JSON.stringify(p.trustIndicators),
            address,
            website,
          }),
          now,
  ...
  const brand: Omit<Brand, 'id' | 'name'> = {
    ...
    ...checkedBrandPresentation({ ...existing?.data, ...d }, config.resources),
    email: email(d.email),
    phone: text(d.phone ?? '', 'Phone', 40, false),
