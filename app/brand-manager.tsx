      ...
      {editor ? (
        <form className="form-stack brand-editor panel" onSubmit={submit}>
          ...
          <fieldset disabled={busy} className="brand-fields">
            <MSection title="Business identity">
            ...
            <MSection title="Client-facing details">
              ...
              {(
                [
                  ['subheading', 'Booking introduction', 1000],
                  ['signature', 'Message signature', 3000],
                  ['footer', 'Proposal and invoice footer', 3000],
                ] as const
              ).map(([key, label, max]) => (
                <SField key={key} label={label + ' (optional)'}>
                  <textarea
                    maxLength={max}
                    value={editor[key]}
                    onChange={(e) => update(key, e.target.value)}
